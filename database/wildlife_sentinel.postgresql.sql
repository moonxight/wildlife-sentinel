...
DO $$ BEGIN
    ...
    END IF;
END $$;
CREATE FUNCTION curdate() RETURNS date LANGUAGE sql STABLE AS 'SELECT CURRENT_DATE';
CREATE FUNCTION field(value text, VARIADIC choices text[]) RETURNS integer LANGUAGE sql IMMUTABLE AS 'SELECT COALESCE(array_position(choices, value), 0)';
CREATE FUNCTION st_geomfromtext(value text) RETURNS point LANGUAGE sql IMMUTABLE STRICT AS $$ SELECT replace(regexp_replace(value, '^POINT\((.*)\)$', '\1', 'i'), ' ', ',')::point $$;
CREATE FUNCTION st_astext(value point) RETURNS text LANGUAGE sql IMMUTABLE STRICT AS $$ SELECT 'POINT(' || value[0] || ' ' || value[1] || ')' $$;
...
BEGIN
 IF TG_OP='INSERT' THEN
   IF NEW.location_geojson IS NULL OR NEW.location_geojson ~= point(0,0) THEN NEW.location_geojson := point(NEW.location_lng, NEW.location_lat); END IF;
...
-- MySQL result-set procedures are PostgreSQL table-returning functions.
...
SELECT * FROM (SELECT *,
    ...
    ORDER BY distance ASC, FIELD(severity,'critical','high','medium','low')
    LIMIT p_limit;
$$;
...
INSERT INTO ws_schema_versions (version) VALUES ('wildlife-postgres-v1');
COMMIT;
